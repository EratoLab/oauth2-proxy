...
| flag: `--insecure-oidc-allow-expired-id-token-on-external-redemption`<br/>toml: `insecure_oidc_allow_expired_id_token_on_external_redemption` | bool | allow expired OIDC ID tokens only on external token redemption when accompanied by a valid access token | false |
