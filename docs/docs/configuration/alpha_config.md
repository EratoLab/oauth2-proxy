...
| `insecureAllowExpiredIDTokenOnExternalRedemption` | _bool_ | InsecureAllowExpiredIDTokenOnExternalRedemption allows an expired ID token<br/>only on the redeem-external-token flow when accompanied by a valid access token.<br/>The access token expiry becomes the session expiry.<br/>default set to 'false' |
